...
IMPORTANT -- the IMU contract matches the EXISTING INS, not a textbook IMU.
    ...
    Noise ownership: dynamics emits CLEAN truth ONLY. Turning truth into a
    noisy measurement is the IMU sensor's job (simulation/sensors/imu.py),
    applied once by the navigation computer. Physics never adds sensor noise --
    that keeps the plant (clean truth) and the sensor model (corruption)
    cleanly separated, with a single home for the IMU error model.
...
class MissileDynamics:
    ...
    def step(
        ...
        if boosting:
            ...
            def deriv(y: np.ndarray) -> np.ndarray:
                lat, alt = y[0], y[2]
                vel = y[3:6]
                accel = self._accel_enu(alt, vel, mass, control,
            ...
            accel0, sf_body0 = self._imu_accelerations(
                state.true_alt, y0[3:6], mass, control, fallback_hat, wind_vel)
        ...
        if boosting:
            ...
            # No control surfaces / no incidence: the airframe points straight
            # along its AIR-relative velocity (in a crosswind the nose is yawed
            # into the wind). Heading + flight-path angle come from that vector.
            new_vel_air = new_vel - wind_vel
            new_yaw = _heading_from_velocity(new_vel_air)
            new_pitch = _gamma_from_velocity(new_vel_air)
        ...
        # Physics owns the PHYSICAL stage: mirror the sequencer's stage into the
        # state so the flight computer can read it from state.py (BOOST->CRUISE
        # at burnout). With no sequencer, preserve whatever the caller set (the
        # sim loop owns PRE_LAUNCHED/launch; guidance owns TERMINAL).
        new_stage = seq.stage if seq is not None else state.missile_stage
    ...
    # ------------------------------------------------------------------
    def _wind_axes(
        self, vel: np.ndarray, fallback_hat: np.ndarray
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray, float]:
        """
        Build the velocity-relative unit triad in ENU.

        Returns (v_hat, side_hat, lift_hat, speed):
            v_hat    : along velocity (body x; the missile points where it goes)
            side_hat : horizontal, to the right of velocity (the `accel_turn` axis)
            lift_hat : perpendicular to V in the vertical plane (the `accel_climb`
                       axis). For x-fwd / y-right / z-down: body_z = -lift_hat.
        """
        ...
        return v_hat, side_hat, lift_hat, speed

    def _force_enu(
        self, alt: float, vel: np.ndarray, mass: float,
        control: ControlInput, fallback_hat: np.ndarray, v_wind: np.ndarray,
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Total ENU force on the missile (whole-airframe point mass).

        Returns (force_no_gravity_enu, gravity_enu, body_axes) where
        body_axes = (v_hat, side_hat, lift_hat). Drag/Mach/q key off the
        AIR-relative velocity v_air = vel - v_wind. The commanded maneuver
        acceleration (perp to velocity) is clamped to the g-envelope and applied
        directly as lift; thrust acts along velocity; force hits ground momentum.
        """
        v_air = vel - v_wind
        v_hat, side_hat, lift_hat, speed = self._wind_axes(v_air, fallback_hat)

        atm = atmosphere.sample(alt)
        mach = speed / atm.speed_of_sound if atm.speed_of_sound > 0 else 0.0
        q_dyn = 0.5 * atm.density * speed * speed

        # Commanded maneuver acceleration (perp to V), clamped to the smaller of
        # the structural g-limit (profile) and the aero-available g (this q).
        a_turn = control.accel_turn
        a_climb = control.accel_climb
        a_mag = math.hypot(a_turn, a_climb)
        a_limit = min(self.profile.get_max_lateral_acceleration(),
                      self.aero.max_lateral_accel(q_dyn, mass))
        if a_mag > a_limit and a_mag > 0.0:
            scale = a_limit / a_mag
            a_turn *= scale
            a_climb *= scale
            a_mag = a_limit

        lift_force = mass * a_mag            # lift the airframe must generate
        drag = self.aero.drag(q_dyn, mach, lift_force)
        thrust = self.engine.thrust(control.throttle, alt)

        # (thrust - drag) along velocity + the commanded maneuver acceleration
        # perpendicular to it. Gravity is added by the caller.
        force_no_g = ((thrust - drag) * v_hat
                      + mass * a_turn * side_hat
                      + mass * a_climb * lift_hat)
        gravity = np.array([0.0, 0.0, -mass * atmosphere.G0])
        return force_no_g, gravity, (v_hat, side_hat, lift_hat)
    ...
    def _accel_enu(
        ...
        force_no_g, gravity, _ = self._force_enu(
            alt, vel, mass, control, fallback_hat, v_wind
        )
